<?php
    ...
    if(isset($busca) && $busca!=""){
        $consulta_dados="SELECT * FROM prog WHERE bet_resultado = 'CASA OU FORA' and (eq_casa LIKE '%$busca%' OR eq_fora LIKE '%$busca%') AND data >= CURDATE() ORDER BY data ASC,league LIMIT $inicio, $registros";

        $consulta_total="SELECT count(id) FROM prog WHERE bet_resultado = 'CASA OU FORA' and (eq_casa LIKE '%$busca%' OR eq_fora LIKE '%$busca%') AND data >= CURDATE()";
    }else{
        $consulta_dados="SELECT * FROM prog WHERE bet_resultado = 'CASA OU FORA' and data >= CURDATE() ORDER BY data ASC,league LIMIT $inicio, $registros";

        $consulta_total="SELECT count(id) FROM prog WHERE bet_resultado = 'CASA OU FORA' and data >= CURDATE()";
    }
    ...
    if($total>=1 && $pagina<=$Npaginas){
        ...
        foreach($dados as $rows){
            ...
            $tabla.='          
                    <td>                        
                        <a href="index.php?vista=casa_fora_bet&id_up='.$rows['id'].'" class="button is-success is-rounded is-small">Apostar</a>
                    </td>
                    <td>
