<?php
    ...
    if(isset($busca) && $busca!=""){
        $consulta_dados="SELECT * FROM prog WHERE O25_resultado = 'BET' AND (eq_casa LIKE '%$busca%' OR eq_fora LIKE '%$busca%') AND data >= CURDATE() ORDER BY data ASC,league LIMIT $inicio,$registros";

        $consulta_total="SELECT count(id) FROM prog WHERE O25_resultado = 'BET' AND (eq_casa LIKE '%$busca%' OR eq_fora LIKE '%$busca%') AND data >= CURDATE()";
    }else{
        $consulta_dados="SELECT * FROM prog WHERE O25_resultado = 'BET' AND data >= CURDATE() ORDER BY data ASC,league LIMIT $inicio,$registros";

        $consulta_total="SELECT count(id) FROM prog WHERE O25_resultado = 'BET' AND data >= CURDATE()";
    }
        ...
        foreach($dados as $rows){
            $tabla.='
                <tr class="has-text-centered" >
                    ...
                    if($rows['eq_home_EXP']+$rows['eq_away_EXP']>=3){
