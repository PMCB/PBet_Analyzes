<?php
    ...
    if(isset($busca) && $busca!=""){
        $consulta_dados="SELECT * FROM prog WHERE (eq_casa LIKE '%$busca%' OR eq_fora LIKE '%$busca%') AND data = DATE_ADD(CURDATE(), INTERVAL 1 DAY) ORDER BY data DESC,league LIMIT $inicio,$registros";

        $consulta_total="SELECT count(id) FROM prog WHERE (eq_casa LIKE '%$busca%' OR eq_fora LIKE '%$busca%') AND data = DATE_ADD(CURDATE(), INTERVAL 1 DAY)";
    }else{
        $consulta_dados="SELECT * FROM prog WHERE data = DATE_ADD(CURDATE(), INTERVAL 1 DAY) ORDER BY data DESC,league LIMIT $inicio,$registros";

        $consulta_total="SELECT count(id) FROM prog WHERE data = DATE_ADD(CURDATE(), INTERVAL 1 DAY)";
    }
