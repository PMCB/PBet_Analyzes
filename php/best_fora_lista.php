<?php
    ...
    if(isset($busca) && $busca!=""){
        $consulta_dados="SELECT * FROM prog WHERE (eq_casa LIKE '%$busca%' OR eq_fora LIKE '%$busca%') AND data >= CURDATE() AND pos_eq_casa-5 > pos_eq_fora ORDER BY data ASC,league LIMIT $inicio,$registros";

        $consulta_total="SELECT count(id) FROM prog WHERE (eq_casa LIKE '%$busca%' OR eq_fora LIKE '%$busca%') AND data >= CURDATE() AND pos_eq_casa-5 > pos_eq_fora";
    }else{
        $consulta_dados="SELECT * FROM prog WHERE data >= CURDATE() AND pos_eq_casa-5 > pos_eq_fora ORDER BY data ASC,league LIMIT $inicio,$registros";

        $consulta_total="SELECT count(id) FROM prog WHERE data >= CURDATE() AND pos_eq_casa-5 > pos_eq_fora";
    }
        ...
        foreach($dados as $rows){
            ...
            if ($rows['eq_away_EXP']>$rows['eq_home_EXP']){
                ...
                $tabla.='<td>'.$rows['eq_away_EXP'].'</td>';
                    ...
                    $tabla.='<td>
                        <a href="index.php?vista=best_fora_bet&id_up='.$rows['id'].'" class="button is-success is-rounded is-small">Apostar</a>
                    </td>
                </tr>
            ';
            $contador++;
