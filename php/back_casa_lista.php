<?php
    ...
    if(isset($busca) && $busca!=""){
        $consulta_dados="SELECT * FROM prog WHERE bet_resultado = 'BACK CASA' and (eq_casa LIKE '%$busca%' OR eq_fora LIKE '%$busca%') AND data >= CURDATE() ORDER BY data ASC,league LIMIT $inicio, $registros";
        ...
        $consulta_dados="SELECT * FROM prog WHERE bet_resultado = 'BACK CASA' and data >= CURDATE() ORDER BY data ASC,league LIMIT $inicio, $registros";
    ...
    if($total>=1 && $pagina<=$Npaginas){
        ...
        foreach($dados as $rows){
            ...
            $tabla.='
                    ...
                    if ($rows['pos_eq_casa_home']<=5)
                    {
                        $tabla.='<td><span class="tag is-success" style="background-color:#C0C0C0;color:black;">'.$rows['pos_eq_casa_home'].'º</span></td>';
                    } else {
                        $tabla.='<td>'.$rows['pos_eq_casa_home'].'º</td>';
                    }
