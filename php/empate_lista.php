<?php
    ...
    if(isset($busca) && $busca!=""){
        $consulta_dados="SELECT * FROM prog WHERE bet_resultado = 'EMPATE' and (eq_casa LIKE '%$busca%' OR eq_fora LIKE '%$busca%') AND data >= CURDATE() ORDER BY data ASC,league LIMIT $inicio, $registros";

        $consulta_total="SELECT count(id) FROM prog WHERE bet_resultado = 'BACK CASA' and (eq_casa LIKE '%$busca%' OR eq_fora LIKE '%$busca%') AND data >= CURDATE()";
    }else{
        $consulta_dados="SELECT * FROM prog WHERE bet_resultado = 'EMPATE' and data >= CURDATE() ORDER BY data ASC,league LIMIT $inicio, $registros";

        $consulta_total="SELECT count(id) FROM prog WHERE bet_resultado = 'BACK CASA' and data >= CURDATE()";
    }
    ...
    if($total>=1 && $pagina<=$Npaginas){
        ...
        foreach($dados as $rows){
            $tabla.='
                <tr class="has-text-centered" >
                    <td>'.$rows['data'].'</td>
                    <td>'.$rows['league'].'</td><td>'.$rows['pos_eq_casa'].'º</td>
                    <td>'.$rows['eq_casa'].'</td>
                    <td>'.$rows['eq_fora'].'</td>
                    ...
                    if ($rows['eq_home_EXP']==$rows['eq_away_EXP']){
                        $tabla.='<td><span class="tag is-success" style="background-color:#90EE90;color:black;">'.$rows['eq_home_EXP'].'</span></td>
                                 <td><span class="tag is-success" style="background-color:#90EE90;color:black;">'.$rows['eq_away_EXP'].'</span></td>';
                    } else {
